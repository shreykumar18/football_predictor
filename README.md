...
For this project, my aim is to develop a predictive model for upcoming football league matches using a comprehensive football dataset. The dataset that I have chosen has a very in-depth analysis on a variety of metrics for both team and player statistics. My goal is to use machine learning algorithms and models that will analyze prior yet recent data found in this dataset and predict the results of future matches, and not only the result but I would also like to predict mostly certain team statistics. Every team has a way or style of playing, obviously not quantifiable but the minor results you get from a certain style like number of shots per half or the xGoals, that is something in which you can see a pattern especially based on the type of opponent you play. What I am hoping to achieve is that models like these can have so many benefits for a large group of people. We can have analysts looking at this both general or working within the football club, something that we now know they heavily do in the modern game, even coaches. And these kind of models also help fans when playing fantasy leagues or even people who are better, it can be a certain statistic that they see that can help them make a decision on a bet. Regardless of whom it might reach , these type of models definitely give each of them a better understanding of the different small factors in a game that might have a big overall impact on the resulting score on any given game.

The way in which my AI agent will be structured is that in general it will use a goal based system in which its main goal will be to predict the result of any forthcoming matches, and other game stats such as xG and shots on target. For the performance measure, this will basically evaluated on the accuracy of predicting the results and these in game statistics to the real games that happen over the period. The Environment is what we have previously talked about such as match data we have as well as how certain opponents tend to perform against certain teams. For the Actuators, this will tie down to the agent and how good it is with probability distributions for all in game statistics. The Sensors I will be using for the agent are: xG, possession, shots on target, previous results against opponents. The dataset that I am using is well equipped to construct such an agent as its structured data for each team and multitude of in-depth stats as well for players help give enough factors for a predictive model of this nature. 
